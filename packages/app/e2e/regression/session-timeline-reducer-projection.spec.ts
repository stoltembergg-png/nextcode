...
test("groups singleton and separated context operations at correct boundaries", async ({ page }) => {
  ...
  for (const id of [
    "prt_boundary_01_read",
    "prt_boundary_02_text",
    "prt_boundary_03_glob",
    "prt_boundary_04_grep",
    "prt_boundary_05_shell",
    "prt_boundary_06_list",
  ]) {
    await expect(page.locator(`[data-timeline-part-id="${id}"]`)).toBeVisible()
  }
  await expect(page.locator('[data-timeline-row="AssistantPart"]')).toHaveCount(6)
})
